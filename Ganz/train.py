...
def main():
	...
	## Data preparation 
	data = support.minst_data() # Load data
	...
	for epoch in range(EPOCHS):
		for n_batch, (real_batch, _) in enumerate(data_loader):
			...
			real_data = Variable(support.image_to_vectors(real_batch))
			if torch.cuda.is_available(): real_data = real_data.cuda()
			# Generating fake data
			fake_data = generator(support.noise(real_data.size(0))).detach()
			# Training the discriminator 
			d_error, d_pred_real, d_pred_fake = train_discriminator(optimizers, 
				real_data, fake_data)
			...
			if (n_batch) % 100 == 0:
				...
				# Display Images
				test_images = support.vector_to_images(generator(test_noise)).data.cpu()
